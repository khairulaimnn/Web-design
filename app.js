...
function subtotal_html(data) {
  ...
  for (let index in cart_items) {
    ...
    html += ` <div class="order-item flex">
                ...
                <p>RM ${(quantity * price) / quantity}</p>
              </div>`;
  }
  html += ` </div>
            <div class="flex">
...
function display_products(data, filters) {
  ...
  for (let index in products) {
    ...
    if (discount > 0) {
      html += `<h4>${price - discount}RM
                  <span style="text-decoration: line-through;color:lightgrey;"> ${price} RM</span>  
                </h4><h4>`;
    } else {
      html += `<h4> ${price}RM</h4><h4>`;
    }
